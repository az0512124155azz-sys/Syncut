...
if (-not (Test-Path -LiteralPath $exe)) { throw "$Label Syncut executable missing: $exe" }
...
$process = Start-Process -FilePath $exe -ArgumentList @('--no-welcome','--config',$configName) -WorkingDirectory $bin -PassThru `
    -RedirectStandardOutput $stdout -RedirectStandardError $stderr
$deadline = (Get-Date).AddSeconds($Seconds)
$seenWindow = $false
$seenTitles = New-Object System.Collections.Generic.List[string]
$badWindowPattern = '(?i)Welcome to Kdenlive|Kdenlive Quick Setup|Syncut setup|\berror\b|\bfatal\b|\bcrash\b|\bcritical\b|שגיאה|קריסה'
$fatalLogPattern = '(?i)Unknown protocol\s*["'']?file|Unable to create KIO worker|Could not load the Qt platform plugin|failed to load.*qml|This application failed to start|Missing package.*Frei0r|no plugins found.*mlt|Failed to open properties file.*profiles|invalid filter.*frei0r'
try {
    while ((Get-Date) -lt $deadline) {
        ...
        if ($titles.Count -gt 0) {
            $seenWindow = $true
            foreach ($title in $titles) { if (-not $seenTitles.Contains($title)) { $seenTitles.Add($title) } }
            $joined = $titles -join ' | '
            Add-Content -LiteralPath $windowsLog -Value (((Get-Date).ToString('o')) + ' | ' + $joined)
            Write-Host $joined
            if ($joined -match $badWindowPattern) { throw "$Label displayed an invalid setup/error window: $joined" }
        }
        $stderrText = Get-Content -LiteralPath $stderr -Raw -ErrorAction SilentlyContinue
        if ($stderrText -match $fatalLogPattern) { throw "$Label logged an incomplete or fatal runtime error." }
    }
    if (-not $seenWindow) { throw "$Label Syncut did not create a visible editor window." }
    if (($seenTitles -join "`n") -match '(?i)Kdenlive') { throw "$Label still exposed Kdenlive window branding." }
    ...
    Stop-Process -Name 'kioworker','kbuildsycoca6','melt','ffmpeg','ffprobe','ffplay','kdenlive_render' -Force -ErrorAction SilentlyContinue
