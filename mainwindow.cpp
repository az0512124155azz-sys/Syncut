...
#include "syncut/aiassistantwidget.h"
...
void MainWindow::init()
{
    ...
    // Syncut AI: functional local assistant. It never installs or downloads a model.
    auto *syncutAiWidget = new SyncutAiAssistantWidget(this);
    auto *syncutAiDock = addDock(i18n("Syncut AI"), QStringLiteral("syncut_ai"), syncutAiWidget, KDDockWidgets::Location_None, m_effectStackDock);
    syncutAiDock->close();
    auto *syncutAiAction = new QAction(QIcon::fromTheme(QStringLiteral("system-search")), i18n("Syncut AI"), this);
    syncutAiAction->setShortcut(QKeySequence(QStringLiteral("Ctrl+Shift+Z")));
    syncutAiAction->setToolTip(i18n("Open Syncut AI (Ctrl+Shift+Z)"));
    addAction(QStringLiteral("syncut_ai"), syncutAiAction);
    connect(syncutAiAction, &QAction::triggered, this, [syncutAiDock]() {
        if (syncutAiDock->isVisible() && !syncutAiDock->visibleRegion().isEmpty()) {
            syncutAiDock->close();
        } else {
            syncutAiDock->open();
            syncutAiDock->setAsCurrentTab();
        }
    });

    // Close non-general docks for the initial Syncut layout.
    // The application opens clean: media bin + monitors + timeline only.
    m_mixerDock->close();
...
void MainWindow::setupActions()
{
    ...
    redo->setShortcut(QKeySequence(QStringLiteral("Ctrl+Y")));
