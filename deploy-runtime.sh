...
# Compatibility mirrors for older code paths that still use *-7 names.
...
# Frei0r is a real runtime plugin tree, not just effect XML metadata.
# Kdenlive/MLT needs these DLLs to expose frei0r.* effects and transitions.
[ -d /ucrt64/lib/frei0r-1 ] || { echo 'ERROR: Frei0r plugin directory missing'; exit 28; }
rm -rf "$INSTALL/lib/frei0r-1"
mkdir -p "$INSTALL/lib/frei0r-1"
cp -R /ucrt64/lib/frei0r-1/. "$INSTALL/lib/frei0r-1/"
[ -f "$INSTALL/lib/frei0r-1/brightness.dll" ] || { echo 'ERROR: Frei0r brightness.dll missing'; exit 29; }
...
# Validate the exact paths that previously failed.
test -n "$(find "$INSTALL/lib/mlt" -maxdepth 1 -type f -iname 'libmlt*.dll' | head -1)" || { echo 'ERROR: MLT plugin DLLs missing'; exit 30; }
test -f "$INSTALL/share/mlt/profiles/dv_pal" || { echo 'ERROR: MLT profile dv_pal missing'; exit 31; }

# Resolve DLL closure after every plugin/executable is present, including Frei0r.
