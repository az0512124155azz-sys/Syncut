...
if (-not (Test-Path -LiteralPath $exe)) { throw "Syncut executable is missing: $exe" }
...
foreach ($required in @($env:MLT_REPOSITORY,$env:MLT_DATA,$env:MLT_PROFILES_PATH,$env:FREI0R_PATH)) {
    if (-not (Test-Path -LiteralPath $required)) { throw "GUI runtime path missing: $required" }
}
...
$seenWindow = $false
$badWindowPattern = '(?i)\berror\b|\bfatal\b|\bcrash\b|\bcritical\b|Kdenlive Quick Setup|Welcome to Kdenlive'
$fatalLogPattern = '(?i)Unknown protocol\s*["'']?file|Unable to create KIO worker|Could not load the Qt platform plugin|failed to load.*qml|This application failed to start|Missing package.*Frei0r|no plugins found.*mlt|Failed to open properties file.*profiles'
...
try {
    while ((Get-Date) -lt $deadline) {
        ...
        if ($titles.Count -gt 0) {
            ...
            Write-Host $joined
            if ($joined -match $badWindowPattern) { throw "$Label Syncut displayed an invalid/error window: $joined" }
        }

        $stderrText = Get-Content -LiteralPath $stderr -Raw -ErrorAction SilentlyContinue
        if ($stderrText -match $fatalLogPattern) { throw "$Label Syncut logged a fatal or incomplete-runtime error." }
    }

    if (-not $seenWindow) { throw "$Label Syncut did not create a visible window." }
    Write-Host "$Label Syncut stayed alive for $Seconds seconds with no Kdenlive setup screen and no runtime error."
} finally {
